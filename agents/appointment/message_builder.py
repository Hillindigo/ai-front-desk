...
class MessageBuilder:
    ...
    def __init__(self):
        self.missing_info_prompts = {
            "gender": "您希望选择哪类服务人员呢？如果有性别偏好也可以告诉我。",
            "start_time": "请问您想预约的时间是？",
            "duration": "请问您需要多长时间的服务？",
            "project": "请问您需要什么服务项目？比如肩颈放松或基础护理。",
            "preference": "您对服务人员的风格、力度或沟通方式有偏好吗？"
        }
    ...
    def create_appointment_success_message(self, tech: Dict[str, Any]) -> str:
        ...
        if tech.get('is_recommendation'):
            ...
            return (f"\nAI Front Desk：已为您预约服务人员：{tech['name']}。预约成功！"
                    f"（原指定的{original_tech.get('name', '')}时间冲突，{tech['name']}在相同服务方面同样合适）"
                    "请按预约时间到店，期待为您服务。\n")
        else:
            return (f"\nAI Front Desk：已为您预约服务人员：{tech['name']}。预约成功！"
                    "请按预约时间到店，期待为您服务。\n")
    ...
    def create_technician_recommendation_message(self, original_tech: Dict[str, Any], 
        ...
        if llm:
            try:
                # 构建LLM提示
                prompt = f"""
作为一个专业的门店预约助手，用户想预约{original_tech['name']}完成{project}，但该服务人员在{start_time}这个时间段不空闲。
...
原服务人员专长：{original_tech.get('strength', '')}

请帮我生成一段温馨、专业的推荐话术，告诉用户原服务人员没空，但推荐人员在相同项目上同样合适，这个时间段有空，询问用户是否愿意预约。
...
"""
        ...
        return (f"\nAI Front Desk：抱歉，{original_tech['name']}在{start_time}这个时间段不空闲。"
                    f"不过{recommended_tech['name']}在{project}方面同样合适，"
                f"这个时间段有空，请问您愿意让我帮您预约吗？\n")
    ...
    def create_recommendation_declined_message(self, llm=None) -> str:
        ...
        if llm:
            try:
                prompt = """
用户拒绝了我推荐的服务人员，请帮我生成一段专业、温馨的回复，表达理解并提供其他选择建议。
...
"""
    ...
    def create_appointment_failure_message(self, technician_name: str) -> str:
        ...
        if technician_name and technician_name != "未知":
            ...
            if specific_tech:
                return f"\nAI Front Desk：抱歉，{technician_name}在您选择的时间段不空闲。请选择其他时间，或者我可以为您推荐其他服务人员。\n"
            else:
                return f"\nAI Front Desk：抱歉，没有找到名为'{technician_name}'的服务人员。请确认姓名，或者我可以为您推荐其他人员。\n"
        else:
            return "\nAI Front Desk：抱歉，该时间段没有合适的服务人员空闲，请选择其他时间或调整偏好。\n"
    ...
    def create_unrelated_message(self) -> str:
        """创建无关请求的消息"""
        return "[REPLY][预约机器人]抱歉，我无法处理这个问题。我只能帮您处理门店服务相关的预约。请问您需要预约服务吗？\n"
