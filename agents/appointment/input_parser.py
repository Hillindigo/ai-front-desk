...
class InputParser:
    ...
    def _create_prompt_template(self) -> PromptTemplate:
        ...
        return PromptTemplate(
            ...
            template=(
                "你是 AI Front Desk 的预约 Agent，负责帮客户预约门店服务。\n"
                f"当前日期是{current_date}，当前北京时间是{current_datetime}。\n"
                "当前已知信息：{history}\n"
                "用户输入：{user_input}\n"
                "特别注意：如果用户输入是对推荐服务人员确认问题的回应（如\"是\"、\"好\"、\"可以\"、\"不\"、\"不要\"等简短回复），请优先识别为confirmation，而不要标记为unrelated。\n"
                "重要：请你只输出纯JSON格式，不要添加任何markdown标记如```json或```，不要添加任何其他文字说明，直接输出JSON：\n"
                "{{\n"
                '  "gender": "服务人员性别偏好（如男/女/未知）",\n'
                ...
                '  "project": "服务项目（如肩颈放松/未知）",\n'
                '  "preference": "用户倾向（如力气大/力气小/无）",\n'
                '  "technician_name": "指定服务人员姓名（如果用户明确提到姓名，如林舟、顾宁等，否则为未知）",\n'
                '  "confirmation": "如果用户在回应服务人员推荐的确认问题，提取用户的回复内容（如是/好/可以/不/不要等），否则为未知",\n'
                '  "info_complete": "根据实际情况判断：1)如果指定了服务人员名且不为未知，需要start_time、project、duration都不为未知；2)如果没指定服务人员名，需要start_time、project、duration、gender都不为未知",\n'
                '  "unrelated": "如果用户的问题和预约无关（如问天气、聊天等），则为true，否则为false。注意：对推荐服务人员的确认回复（是/不等）不应标记为unrelated",\n'
                '  "missing_info": "如果info_complete为false，请列出缺少的关键信息，如[start_time, project]等"\n'
                "}}\n"
                "判断逻辑：\n"
                "1. 如果用户明确指定了服务人员姓名（如\"林舟\"、\"预约顾宁\"等），请务必提取technician_name\n"
                "2. 如果用户在回应推荐服务人员的确认问题（如回复\"是\"、\"好\"、\"可以\"、\"不\"、\"不要\"等），请提取到confirmation字段，并且不要将其标记为unrelated\n"
                "3. 必需信息判断：\n"
                "   - 如果指定了服务人员名：需要start_time、project、duration\n"
                "   - 如果没指定服务人员名：需要start_time、project、duration、gender\n"
