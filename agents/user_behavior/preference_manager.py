...
class PreferenceManager:
    ...
    def update_technician_preference(self, technician_id: int):
        ...
        try:
            ...
            self.logger.info(f"更新服务人员偏好: {technician_id}")
        except Exception as e:
            self.logger.error(f"更新服务人员偏好失败: {str(e)}")
    ...
    def update_technician_type_preference(self, technician_type: str):
        ...
        Args:
            technician_type: 服务人员类型偏好（如：力气大、手法轻等）
        """
        try:
            self.behavior_db.update_user_preference('technician_type', technician_type)
            self.logger.info(f"更新服务人员类型偏好: {technician_type}")
        except Exception as e:
            self.logger.error(f"更新服务人员类型偏好失败: {str(e)}")
    ...
    def get_preferred_technician_id(self) -> Optional[int]:
        ...
        try:
            ...
            self.logger.error(f"获取偏好服务人员ID失败: {str(e)}")
            return None
