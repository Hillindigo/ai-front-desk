...
class TestAppointmentAgentCoreFeatures:
    ...
    def test_should_extract_user_info_from_natural_language(self):
        ...
        # 验证解析结果包含预期信息
        assert result["project"] == "肩颈放松", f"应该提取到肩颈放松项目，但得到：{result['project']}"
        assert result["gender"] == "女", f"应该提取到女服务人员偏好，但得到：{result['gender']}"
    ...
    def test_should_track_appointment_state_correctly(self):
        ...
        用户分步骤提供信息：
        1. "我要预约肩颈放松" -> 应该记录项目=肩颈放松，其他为空
        2. "明天下午2点" -> 应该记录时间，保持项目=肩颈放松
        3. "女服务人员" -> 应该记录性别偏好，保持之前信息
        """
        agent = AppointmentAgent()
        ...
        assert agent.appointment_history["project"] == "肩颈放松"
        ...
        assert agent.appointment_history["project"] == "肩颈放松"  # 应该保持
...
class TestAppointmentAgentEdgeCases:
    ...
    def test_should_reset_state_properly(self):
        ...
        # 设置一些状态
        agent.appointment_history["project"] = "肩颈放松"
