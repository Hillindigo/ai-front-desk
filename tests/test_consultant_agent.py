...
class TestConsultantAgentCoreFeatures:
    ...
    @pytest.mark.asyncio
    async def test_should_answer_service_related_questions(self):
        """测试：应该能回答肩颈放松相关问题"""
        ...
        for question in service_questions:
            ...
            service_keywords = ["肩颈放松", "service", "效果", "好处", "肌肉", "血液", "放松"]
            has_relevant_content = any(keyword in response for keyword in service_keywords)
            assert has_relevant_content, f"回答应该与肩颈放松相关，问题：{question}，回答：{response[:200]}..."
    ...
    async def test_should_search_knowledge_from_database(self):
        ...
        if len(knowledge_results) > 0:
            ...
            service_keywords = ["肩颈放松", "service", "效果", "好处", "肌肉"]
            has_relevant = any(keyword in all_content for keyword in service_keywords)
            assert has_relevant, f"检索的知识应该与肩颈放松相关，但得到：{all_content[:300]}..."
    ...
    async def test_should_generate_professional_response(self):
        ...
        question = "肩颈放松对身体有什么作用？"
        response = await agent.consult(question)
        
        assert len(response) > 50, f"专业回答应该有一定长度，但只有{len(response)}字符：{response}"
        assert not response.startswith("抱歉"), f"对于肩颈放松相关问题不应该道歉开头：{response[:100]}..."
...
class TestConsultantAgentEdgeCases:
    ...
    async def test_should_work_with_stream_mode(self):
        ...
        service_keywords = ["肩颈放松", "好处", "效果", "肌肉", "血液"]
        stream_has_content = any(keyword in stream_response for keyword in service_keywords)
        normal_has_content = any(keyword in normal_response for keyword in service_keywords)
