...
class TestUserBehaviorAgentCoreFeatures:
    ...
    def test_should_record_user_behavior_correctly(self):
        ...
        try:
            ...
            assert latest_behavior["service_type"] == "肩颈放松"
    ...
    def test_should_analyze_user_preferences_correctly(self):
        ...
        behaviors = [
            {"user_id": user_id, "action": "appointment", "service_type": "肩颈放松", "preferred_time": "下午"},
            {"user_id": user_id, "action": "appointment", "service_type": "肩颈放松", "preferred_time": "下午"},
            {"user_id": user_id, "action": "appointment", "service_type": "深度放松", "preferred_time": "上午"},
            {"user_id": user_id, "action": "consultation", "topic": "肩颈放松效果"},
        ]
        ...
        try:
            ...
            # 应该识别出肩颈放松是最常选择的服务（这个测试可能会失败）
            preferred_service = preferences.get("preferred_service_type")
            assert preferred_service == "肩颈放松", f"应该识别肩颈放松为偏好服务，但得到：{preferred_service}"
    ...
    def test_should_generate_personalized_recommendations(self):
        ...
        historical_behaviors = [
            {"user_id": user_id, "service_type": "深度放松", "satisfaction_rating": 5},
            {"user_id": user_id, "service_type": "肩颈放松", "satisfaction_rating": 4},
        ...
        try:
            ...
            service_related = any("肩颈放松" in service for service in recommended_services)
            assert service_related, f"推荐应该包含肩颈放松相关服务，但得到：{recommended_services}"
...
class TestUserBehaviorAgentDataManagement:
    ...
    def test_should_manage_user_preferences_storage(self):
        ...
        # 设置用户偏好
        preferences = {
            "preferred_service_types": ["肩颈放松", "深度放松"],
        ...
        try:
            ...
            assert "肩颈放松" in retrieved_prefs["preferred_service_types"]
    ...
    def test_should_handle_data_aggregation(self):
        ...
        # 创建多个用户的数据
        users_data = [
            {"user_id": "user_1", "service_type": "肩颈放松", "satisfaction": 5},
            {"user_id": "user_2", "service_type": "肩颈放松", "satisfaction": 4},
            {"user_id": "user_3", "service_type": "深度放松", "satisfaction": 3},
            {"user_id": "user_1", "service_type": "肩颈放松", "satisfaction": 5},
        ]
        ...
        try:
            ...
            assert "肩颈放松" in str(service_stats), "聚合结果应该包含肩颈放松统计"
            assert "深度放松" in str(service_stats), "聚合结果应该包含深度放松统计"
