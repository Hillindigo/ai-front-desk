...
> 本文定位：Phase B 的问题分析、修复方案、执行任务和验收标准，不代表代码已经完成。
...
- [ ] 可以创建会话并获得唯一 `conversation_id`。
- [ ] 用户消息和 Agent 消息都能持久化到正确会话。
- [ ] 两个会话之间不共享消息历史、预约草稿、Agent 运行时状态或锁。
- [ ] 服务重启后可以恢复会话元数据和最近消息。
- [ ] 同一会话并发请求不会造成消息顺序或状态覆盖错误。
- [ ] 旧 `/chat/stream` 仍可用，新会话 API 可用。
- [ ] 前端刷新后可以继续原会话，创建新会话后不会串历史。
...
- [ ] 新增服务重启恢复测试通过。
- [ ] 新增同会话并发测试通过。
- [ ] 测试使用临时数据库，不污染共享本地数据库。
- [ ] Fake 模式下测试不调用真实 LLM 或 Embedding API。
- [ ] 所有数据库路径均来自 `db_config`，没有业务代码或 fixture 继续硬编码数据库路径。
- [ ] 每个 turn 的 DB Session 均独立、短生命周期，commit/rollback/close 边界有测试证据。
- [ ] FakeLLM 预设表覆盖 B6 所需的多会话、多轮和错误边界场景。
- [ ] `user_behavior` 的既有 skip 原因仍被准确记录，不能伪装成通过。

### 7.3 运行和文档完成

- [ ] 没有预先创建 `data/` 目录时，Fake 模式可以初始化。
- [ ] 无真实模型 key 时，应用启动失败边界已明确，不能出现无解释的崩溃。
- [ ] README 已说明会话 API、旧接口兼容策略和本地运行方式。
- [ ] Phase B 执行结果记录了实际命令、通过/跳过/失败数量和警告。
- [ ] 剩余的 A-R1、A-R2、A-R3、A-R5 状态已更新并说明去向。
- [ ] `git diff --check` 通过，工作区只包含 Phase B 范围内的改动。
