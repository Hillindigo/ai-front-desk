...
class UserBehaviorAgent:
    ...
    async def generate_personalized_reminder(self, user_id: str = "default_user", 
        ...
        try:
            ...
            tech_name = tech_info.get('name', '您偏爱的服务人员') if tech_info else '您偏爱的服务人员'
            tech_strength = tech_info.get('strength', '') if tech_info else ''
            service = analysis.get('favorite_service', '门店服务')
            duration = analysis.get('favorite_duration', 60)
            
            self.logger.info(f"服务人员信息: {tech_name}, 专长: {tech_strength}")
            ...
            # 构建LLM提示
            prompt = f"""请为线下服务门店生成一条温暖的回访消息。

客户信息：
- 称呼：尊敬的Tom
- 最喜欢的服务人员：{tech_name}
- 服务人员专长：{tech_strength}
- 常用服务：{service}
- 常用时长：{duration}分钟
- 理疗师空闲时间：{times_text}
...
7. 直接输出消息内容，不要任何标记"""
    ...
    async def get_reminder_with_schedule(self, user_id: str = "default_user") -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "message": "您好！系统暂时无法查询服务人员时间，请稍后再试或直接联系我们预约。",
                "technician_available_times": []
            }
