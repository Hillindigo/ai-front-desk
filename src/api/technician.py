...
router = APIRouter(prefix="/api/technicians", tags=["服务人员管理"])
...
@router.get("/", response_model=List[TechnicianResponse], summary="获取所有服务人员")
async def get_all_technicians():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取服务人员信息失败: {str(e)}")


@router.get("/{technician_id}/schedule", response_model=List[ScheduleResponse], summary="获取服务人员排班")
...
@router.get("/{technician_id}", response_model=TechnicianResponse, summary="获取单个服务人员信息")
...
@router.get("/schedules/today", summary="获取所有服务人员今日排班")
