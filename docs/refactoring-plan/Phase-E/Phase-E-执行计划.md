...
- [x] 存在统一的 `ContextBuilder` 和结构化 `ContextPackage`。
- [x] 上下文有配置化预算、固定优先级和可替换 Token 估算器。
- [x] 当前输入、活跃预约、待确认动作和关键工作流状态不会因裁剪丢失。
- [x] 摘要记录会话、覆盖序号、版本、状态和生成来源。
- [x] 摘要成功可恢复，失败保留旧版本和原始消息，不阻断主对话。
- [x] 用户偏好有来源、可信度、确认时间和删除语义。
- [x] 偏好 API 可以查看、修改和删除当前用户的偏好；删除后下一轮通过墓碑、摘要失效和来源消息屏蔽立即不再使用；前端有成功/失败/删除文案反馈。独立偏好管理页面不作为门槛。
- [x] 旧偏好数据经过来源审查，未确认记录不会静默成为长期事实。
- [x] 统一编排链路使用 ContextBuilder，Agent 不自行拼装全量历史或直接写长期偏好。
- [x] 知识证据带文档 ID、分数、分类和索引版本，并经过阈值过滤。
- [x] 知识索引重建与查询不会产生索引/ID 映射不一致。
- [x] Phase D 的 SSE v1、错误语义和预约领域状态机保持兼容。

### 7.2 测试完成

- [x] Phase B/C/D 原有测试无新增回归。
- [x] ContextBuilder、预算、摘要覆盖、版本和回退测试通过。
- [x] 摘要重启恢复、同会话并发和故障注入测试通过。
- [x] 偏好隔离、来源、覆盖、删除和重复请求测试通过。
- [x] 预约事实不可被摘要或模型输出替换的测试通过。
- [x] 知识阈值、引用、索引重建并发和无依据降级测试通过。
- [x] Fake 模式下无真实 LLM/Embedding 请求，测试使用临时数据库。
- [x] 既有 10 个 `user_behavior` skip 的状态和原因被准确记录。
- [x] 通过 TestClient 和最小 HTTP 请求验证 turns、偏好接口和断线/重试边界。

### 7.3 运行和文档完成

- [x] 新数据库可初始化，摘要和偏好数据的迁移/兼容策略已记录并验证。
- [x] README、`PROJECT_MEMORY.md` 和本文件已同步实际接口、边界和完成状态。
- [x] `git diff --check` 通过，阶段提交未混入无关工作区改动。
- [x] 报告明确区分本地自动化、Fake 模式、真实模型、线上、鉴权和多进程证据。
- [x] 未完成的摘要质量、真实模型稳定性、生产鉴权和多租户隔离不能标记为已完成。
- [x] Phase F 的知识库后台、索引治理和运营能力交接项已形成清单。
